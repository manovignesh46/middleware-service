...
export class NotificationServiceClient {
  ...
  async sendNotification(sendNotificationServiceDto: SendNotificationDto) {
    ...
    sendNotificationServiceDto.sourceMicroservice = SourceMicroservice.AUTH; //always set sourceMicroservice to Autth
    ...
    try {
      ...
      if (response?.status === ResponseStatusCode.SUCCESS) {
        ...
        throw new Error(response?.message || 'Error Sending Notification');
      }
    } catch (err) {
