...
describe('OTPExceptionFilter', () => {
  ...
  describe('All exception filter tests', () => {
    ...
    it('OTP Expired Error should return appropriate response', () => {
      ...
      service.catch(new OtpExpiredError(exceptionMessage), mockArgumentsHost);
  ...
  it('OTP Locked Error should return appropriate response', () => {
    const exceptionMessage = 'Some Exception';
    service.catch(new OtpLockedError(exceptionMessage, 10), mockArgumentsHost);
    expect(mockHttpArgumentsHost).toBeCalledTimes(1);
    expect(mockHttpArgumentsHost).toBeCalledWith();
    expect(mockGetResponse).toBeCalledTimes(1);
    expect(mockGetResponse).toBeCalledWith();
    ...
    expect(mockJson).toBeCalledWith(
      new StatusMessagePresenter(
        ...
        ResponseMessage.OTP_LOCK.replace('${timeInMinutes}', '10'),
      ),
    );
  });
  it('OTP Not Locked Error should return appropriate response', () => {
    const exceptionMessage = 'Some Exception';
    service.catch(new OtpNotExpiredError(exceptionMessage), mockArgumentsHost);
    expect(mockHttpArgumentsHost).toBeCalledTimes(1);
    expect(mockHttpArgumentsHost).toBeCalledWith();
    expect(mockGetResponse).toBeCalledTimes(1);
    expect(mockGetResponse).toBeCalledWith();
